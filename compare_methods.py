...
def load_ace_results():
    ...
    try:
        ...
        if len(final_ep) == 0:
            print(f"⚠️  No final episode found in {metrics_file}")
            return None
        ...
        print(f"⚠️  Error loading ACE results: {e}")
        return None
...
def print_comparison_table():
    ...
    if ace:
        ...
        if ace['total'] < best_baseline['total']:
            ...
            print(f"✅ ACE BEATS best baseline by {improvement:.1f}%")
        elif ace['total'] < 2.0:
            print(f"✅ ACE COMPETITIVE with baselines (better than Random, Max-Var)")
        else:
            print(f"⚠️  ACE underperforms (worse than best baseline)")
        print()
        ...
        if ace['X3'] < best_baseline_x3:
            ...
            print(f"  ✅ Superior collider identification ({improvement:.1f}% better)")
        elif ace['X3'] < 0.5:
            print(f"  ✅ Collider learned successfully (< 0.5 threshold)")
        else:
            print(f"  ⚠️  Collider not well learned")
        print()
        ...
        if episode_reduction >= 70:
            print(f"  ✅ Supports '80% reduction' claim (close enough)")
        elif episode_reduction >= 40:
            print(f"  ⚠️  {episode_reduction:.0f}% reduction (not quite 80%, but significant)")
        else:
            print(f"  ⚠️  Only {episode_reduction:.0f}% reduction - update claim")
        print()
        
        # Success criteria check
        print("Success Criteria (from guidance doc):")
        print(f"  X3 < 0.5: {'✅' if ace['X3'] < 0.5 else '❌'} ({ace['X3']:.4f})")
        print(f"  X2 < 1.0: {'✅' if ace['X2'] < 1.0 else '❌'} ({ace['X2']:.4f})")
        print(f"  X5 < 0.5: {'✅' if ace['X5'] < 0.5 else '❌'} ({ace['X5']:.4f})")
        print(f"  X1 < 1.0: {'✅' if ace['X1'] < 1.0 else '❌'} ({ace['X1']:.4f})")
        print(f"  X4 < 1.0: {'✅' if ace['X4'] < 1.0 else '❌'} ({ace['X4']:.4f})")
