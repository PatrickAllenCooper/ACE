#!/bin/bash
#SBATCH --job-name=duffing_baselines
...
#SBATCH --output=logs/duffing_baselines_%j.out
#SBATCH --error=logs/duffing_baselines_%j.err

# Duffing Oscillators Baseline Comparison
# Run Random, Round-Robin, Max-Variance for N=5 seeds
# Compare against existing ACE results (0.042 ± 0.036)

echo "=============================================="
echo "Duffing Baselines (3 methods × 5 seeds)"
...
OUTPUT_DIR="results/duffing_baselines_${TIMESTAMP}"
mkdir -p "$OUTPUT_DIR"

# Run baselines for each seed
for SEED in 42 123 456 789 1011; do
    for METHOD in random round_robin max_variance; do
        echo ""
        echo "Running Duffing $METHOD seed $SEED..."
        
        python -u experiments/duffing_oscillators.py \
            --policy $METHOD \
            --episodes 100 \
            --seed $SEED \
...
csvs = glob.glob('$OUTPUT_DIR/*/duffing_results.csv')
if csvs:
    results = []
    for f in csvs:
    ...
    summary.to_csv('$OUTPUT_DIR/duffing_baselines_summary.csv', index=False)
...
echo ""
echo "=============================================="
echo "Duffing baselines complete"
echo "Completed: $(date)"
echo "Results: $OUTPUT_DIR/duffing_baselines_summary.csv"
echo "=============================================="
