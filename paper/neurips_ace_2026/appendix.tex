...
\item \textbf{Synthetic SCMs (5-node; hierarchical 30/50-node).} Nodes $X_1\dots X_N$; interventions $\mathrm{do}(X_i{=}\nu)$ over $[-5,5]$. The hierarchical family adds layered colliders; the anonymised variant strips semantics.
\item \textbf{Physics --- coupled Duffing oscillators.} Nodes are the oscillator displacements in a chain $X_1\!\leftrightarrow\!X_2\!\leftrightarrow\!X_3$; an intervention is a \emph{clamp} $\mathrm{do}(X_\text{mid}{=}0)$ that physically pins the intermediate mass and breaks the synchronisation-induced spurious correlation. The mechanisms are ODE-derived but the prompt sees the same edge list and per-node losses.
\item \textbf{Economics --- US Phillips curve (FRED).} Nodes are real macroeconomic series (\texttt{UNRATE}, \texttt{FEDFUNDS}, \texttt{MICH}, \texttt{CPILFESL}); ``intervention'' is active selection of a historical data subset rather than a physical $\mathrm{do}$ (stated as such in the main text). Here the semantically meaningful names let the LM's pretrained economic prior engage most strongly --- the opposite end of the spectrum from the anonymised SCM control.
\end{itemize}
The anonymised-vs-named contrast across these modalities is what the LM-prior-vs-DPO analysis (Appendix~\ref{app:scaling}) exploits: performance is preserved even when names carry no meaning, indicating the structural-plus-loss reasoning, not name semantics alone, drives intervention choice.
...
We isolate the pretrained LM's contribution from the DPO updates by comparing ACE against \emph{ACE without DPO}: the identical LM proposer and lookahead-select mechanism with only the DPO weight updates disabled. This differs from the ``No DPO'' row of Table~\ref{tab:ablations}, which additionally replaces the LM with random proposals. Under anonymisation, canonical node names $X_1\dots X_N$ are replaced by abstract hex tokens (\texttt{n\_xxxx}) drawn deterministically from the seed, removing any natural-language semantics the LM could exploit and leaving only graph structure and per-node losses to reason from.
...
\caption{\textbf{LM prior vs.\ DPO under anonymisation (30 nodes) and scaling reference (50 nodes).} (a)~Best-MSE learning curves on the anonymised 30-node SCM: ACE and ACE-without-DPO (LM proposer + lookahead, no weight updates) are statistically indistinguishable; the dashed line marks the mean episode at which best-MSE was first reached. The anonymised ACE runs were truncated by the wall-time limit at $\sim$episode 19, but best-MSE had already plateaued by $\sim$episode 20 for both methods, so the best-MSE comparison is fair. (b)~\emph{Per-node} best MSE (total/$N$), which is scale-invariant: the 50-node cells are a larger, harder graph, not a worse method, so the per-node normalisation is what makes the comparison meaningful across $N$. ``ACE w/o DPO'' is an ablation of ACE, not naive zero-shot prompting.}
\label{fig:followup}
\end{figure}

The two variants are tied on canonical 30-node SCMs (ACE $1.95\pm0.77$ vs.\ ACE-w/o-DPO $1.73\pm0.22$) and remain tied under anonymisation (ACE $1.49\pm0.16$ vs.\ ACE-w/o-DPO $1.49\pm0.18$, $\approx\!0.050$ per node). At this scale the pretrained prior plus lookahead carries most of the load; DPO's marginal value is expected to grow where the prior is weaker. The scaling pipeline of Section~\ref{sec:limitations} (compact salience-ranked prompt encoding, per-node metric, plateau-budgeted episodes with checkpoint--resume, and the $K$/capacity ablations at 50 nodes) targets exactly that regime; the $100+$ node configuration is specified as future work.
