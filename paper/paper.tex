...
\begin{abstract}
Discovering causal relationships requires controlled experiments, but which experiment should come next? Static heuristics cannot adapt as knowledge accumulates. We propose Active Causal Experimentalist (ACE), which learns experimental design strategies via Direct Preference Optimization. By learning from pairwise comparisons rather than estimating value functions, ACE achieves stable training under the non-stationary rewards inherent to scientific discovery. We introduce per-node convergence criteria and dedicated root learners to handle heterogeneous learning rates across mechanisms. Across synthetic benchmarks, physics simulations, and economic data, ACE achieves 70--71\% improvement over baselines at matched intervention budgets with strong statistical significance (p<0.001, Bonferroni corrected) and large effect sizes (Cohen's $d \approx 2$, indicating the difference exceeds two standard deviations—a practically meaningful improvement beyond mere statistical significance). Comprehensive ablation studies validate that all four architectural components contribute meaningfully (40--130\% degradation when removed). ACE without oracle pretraining still outperforms all baselines, confirming that learned experimental design, not privileged initialization, drives performance.
\end{abstract}
...
\begin{equation}
\mathcal{L}_{\text{DPO}}(\pi_\phi) = - \mathbb{E}_{(\sigma, y_w, y_l)} \left[ \log \sigma \left( \beta \left[ \log \frac{\pi_\phi(y_w \mid \sigma)}{\pi_{\text{ref}}(y_w \mid \sigma)} - \log \frac{\pi_\phi(y_l \mid \sigma)}{\pi_{\text{ref}}(y_l \mid \sigma)} \right] \right) \right]
\end{equation}
where $y_w$ and $y_l$ are preferred and dispreferred interventions, $\pi_{\text{ref}}$ is the reference policy, and $\beta = 0.1$ is the temperature parameter controlling deviation from the reference.
