...
Crucially, ACE learns from the outcomes of experiments: did intervening on gene A provide more information than gene B? Did testing reactant concentration C help more than temperature T? By training via Direct Preference Optimization (DPO) \cite{rafailov2023direct} on such pairwise comparisons, ACE learns adaptive strategies without requiring explicit value function estimation---a critical advantage given the non-stationary nature of scientific discovery (the "best" next experiment changes as our knowledge grows).
...
\begin{itemize}
    \item Structural constraints: Known presence or absence of specific edges, forbidden parent sets due to temporal ordering or physical constraints
    \item Parametric constraints: Bounds on effect sizes, monotonicity assumptions, or known symmetries (e.g., $\theta_{12} = \theta_{13}$ in symmetric systems)
    \item Functional knowledge: Known linearity or non-linearity of specific relationships, conservation laws, or equilibrium conditions
    \item Operational constraints: Variables that cannot be intervened upon (e.g., customer demographics in A/B tests), cost asymmetries (clinical trials vs. observational data), safety boundaries, or regulatory limits on experimental scope
\end{itemize}
...
DPO sidesteps this issue entirely by learning from rankings rather than magnitudes. The relative ordering---which of two interventions is better---remains stable even as absolute rewards decay. This invariance to reward scale makes DPO naturally suited to scientific discovery where diminishing returns are unavoidable.

Why Preference Learning Works for Experimental Design. The theoretical justification stems from the Bradley-Terry model underlying DPO: preference probabilities $P(y_w \succ y_l \mid s)$ depend only on reward differences, not absolute magnitudes. When rewards are non-stationary (shifting over time), differences can remain stable even as individual rewards change. Formally, if $r_t(a) = f(t) \cdot r_0(a)$ where $f(t)$ is a time-varying scale factor, the preference $P(a \succ b)$ depends on $r_0(a) - r_0(b)$, which is invariant to $f(t)$. This scale-invariance property provides robustness to the inherent non-stationarity of experimental design, where information gain decays as $\mathcal{O}(1/t)$ with accumulated evidence \cite{murphy2001active}.
