...
Results. Across 5 independent runs on coupled Duffing oscillators, the system achieves a final loss of 0.042 $\pm$ 0.036, demonstrating successful recovery of the chain coupling structure. Interventions on intermediate oscillators decouple the synchronized system, breaking spurious correlations and enabling structure identification from observational data that initially suggests full connectivity due to synchronization dynamics.
...
Baseline performance hierarchy: Max-Variance (2.05 $\pm$ 0.12) outperforms PPO (2.11 $\pm$ 0.13), Round-Robin (2.15 $\pm$ 0.08), and Random (2.18 $\pm$ 0.06), with all differences statistically significant at $p < 0.05$ via paired t-tests. This demonstrates that uncertainty sampling provides measurable advantages over both unguided exploration and value-based RL for mechanism learning. \textcolor{red}{[TODO: Add ACE comparison showing competitive performance with fewer episodes]}

\section{Discussion}

\subsection{Ablation Studies}

We systematically ablate each component and measure performance degradation. \textcolor{red}{[TODO: Fill with ablation results]} Per-node convergence prevents premature termination, the dedicated root learner enables exogenous variable learning, and diversity reward prevents policy collapse. Each component addresses a specific failure mode.
...
ACE excels when causal structures contain colliders or complex dependencies requiring strategic intervention allocation, experimental budgets are moderate (20-200 experiments) allowing policy learning, mechanisms have heterogeneous learning rates benefiting from per-node convergence, and some structural knowledge exists (known or hypothesized graph).
