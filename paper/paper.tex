...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw, rounded corners, minimum width=2.6cm, minimum height=0.8cm, font=\scriptsize, thick, align=center},
...
% DPO arrow
\draw[arrow] (sim) -- node[annot, above] {preferences} (dpo);

% Feedback loops
\draw[arrow, dashed, gray] (dpo.north) -- ++(0,0.6) -| (state.east);
\draw[arrow, dashed, gray] (learn.west) -- ++(-1.2,0) |- (state.west);

% Side annotations (positioned to avoid overlap)
\node[annot, left=0.8cm of gen] {LLM policy};
\node[annot, right=0.3cm of dpo.south east, anchor=north west] {$R = \Delta\mathcal{L} + \alpha w + \beta D$};
