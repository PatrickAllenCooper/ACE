...
\textbf{Scaling to 15 nodes.} To test ACE's scalability, we applied it to the complex 15-node SCM with 11 colliders. ACE achieved 4.54 total MSE (seed 42, 300 episodes), competitive with baselines: Greedy Collider (4.51 $\pm$ 0.17), Random (4.62 $\pm$ 0.18), and PPO (4.68 $\pm$ 0.20). While ACE's advantage is less pronounced on this larger structure (baseline-competitive vs 70\% improvement on 5 nodes), it demonstrates that the learned strategy generalizes to more complex systems without architectural modification.
...
We validate ACE's key architectural component through ablation. Table~\ref{tab:ablations} shows that removing the diversity reward causes substantial performance degradation (206\%), reducing ACE to baseline-level performance. This confirms that exploration diversity is essential for ACE's strategic behavior, complementing the DPO-based proposal generation validated by the lookahead ablation (Section~\ref{sec:results}).

\begin{table}[t]
\centering
\caption{Diversity reward ablation (N=2 seeds, 100 episodes). Removing diversity causes substantial degradation to near-baseline levels, validating its necessity for strategic exploration.}
\label{tab:ablations}
\footnotesize
\begin{tabular}{@{}lcc@{}}
\toprule
Configuration & Loss & Change \\
\midrule
ACE (Full) & 0.92$\pm$0.73 & -- \\
\quad median & 0.61 & -- \\
\midrule
No Diversity Reward & 2.82$\pm$0.22 & +206\% \\
\quad (N=2, seeds 42, 123) & & \\
\midrule
\multicolumn{3}{@{}l}{\textit{Baselines: 2.03--2.10 (equal episodes)}} \\
\bottomrule
\end{tabular}
\end{table}

Without diversity reward, ACE degrades to baseline performance (2.82 vs 2.03--2.10). The policy over-concentrates interventions on a single node, preventing balanced mechanism learning. Combined with the lookahead ablation (which proved DPO's contribution), this demonstrates that ACE's advantage derives from both learned proposal generation and strategic exploration diversity.
