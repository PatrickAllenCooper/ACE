...
Our contributions are threefold: \textbf{(1)} We introduce a simplified, three-component reward formulation that balances information gain, node importance, and exploration diversity; \textbf{(2)} We propose per-node convergence criteria and dedicated root learners to address the heterogeneous learning rates of exogenous and endogenous variables; \textbf{(3)} We demonstrate on synthetic and real-world domains that preference-based learning provides a stable alternative to value-based RL for experimental design, achieving 80\% computational savings while maintaining competitive performance.
...
\begin{itemize}
    ...
    \item \textit{Measurement limitations}: Variables that cannot be intervened upon, cost constraints on different interventions, or maximum sample sizes
\end{itemize}
...
\textbf{Simplified Reward Function.}
...
Exogenous variables (roots with no parents) present a fundamental challenge: under intervention $\text{do}(V_i = x)$, the natural distribution $P(V_i)$ is never observed. Interventional data provides no signal for learning root distributions. Initial experiments showed root nodes failed to converge (losses remained at initialization) while mechanisms with parents learned successfully.

We address this via a \textit{dedicated root learner}---a separate module trained exclusively on observational data $\mathcal{D}^{\text{obs}} \sim P_{M^*}(\mathcal{V})$ with no interventions. For root nodes, this learner estimates parameters $(\mu_i, \sigma_i)$ via maximum likelihood and periodically transfers these estimates to the main student model. This isolation ensures root distributions are learned from appropriate data.

\textbf{Per-Node Convergence Criteria.}
Mechanisms exhibit heterogeneous learning rates: linear relationships converge in 5-10 episodes, quadratic functions require 40-50 episodes, and roots need specialized treatment. Global early stopping based on aggregate metrics (e.g., fraction of zero-reward steps) terminates prematurely, stopping when fast learners converge but slow learners remain incomplete.

We introduce per-node convergence monitoring: the system tracks whether each node $V_i$ has achieved its target loss $L_i < \tau_i$ for $p$ consecutive episodes. Training terminates only when all nodes simultaneously satisfy their criteria. This ensures complete mechanism recovery before stopping, preventing the pathology observed in initial tests where the system stopped at episode 8 with quadratic mechanisms unconverged (loss 0.898 vs. target 0.5).
...
\subsection{Simplicity Versus Complexity}

Our implementation evolved through multiple iterations, initially employing 11 reward components with complex interactions. Analysis revealed this complexity was counterproductive: a baseline using simple uncertainty sampling (Max-Variance) matched or exceeded ACE's performance. This motivated aggressive simplification to a 3-component reward (information gain, node importance, diversity), reducing complexity by 60\% while maintaining performance.

This finding aligns with the broader machine learning principle that simpler models with fewer hyperparameters often generalize better. The key insight is that \textit{the right objective matters more than complex reward shaping}. DPO's preference-based objective naturally handles the core challenge (non-stationary rewards), making elaborate bonuses unnecessary.

\subsection{Lessons for Active Learning in Science}

Our results suggest several principles for designing active learning systems for scientific discovery:

\begin{enumerate}
\item \textbf{Prefer rankings over magnitudes} when reward signals are non-stationary
\item \textbf{Respect heterogeneous timescales} with per-component convergence criteria
\item \textbf{Isolate observational learning} for components (like roots) where interventions provide no signal
\item \textbf{Simplify reward functions} to interpretable, low-dimensional objectives
\end{enumerate}

These principles emerged not from theory but from empirical iteration and careful analysis of failure modes, highlighting the value of rigorous experimental methodology even in developing experimental methodologies themselves.
...
Through systematic simplification from an initial 11-component reward to a 3-component formulation, we demonstrated that simpler objectives often outperform complex reward shaping. This finding has broader implications for active learning in science: the right learning algorithm (DPO's preference-based objective) combined with domain-appropriate architectural choices (dedicated root learner, per-node convergence) can be more effective than elaborate reward engineering.
...
Future work includes extending ACE to structure discovery (learning the graph topology itself), scaling to larger causal systems, and deployment in real-world scientific domains where simulation costs make sample efficiency critical. The framework's ability to leverage pretrained language models suggests promising directions for incorporating natural language domain knowledge and scientific literature to guide experimental design.

\section{Limitations and Future Work}

\textbf{Simulation Dependence.} ACE requires high-fidelity simulations during policy training (hundreds of episodes with thousands of forward evaluations). While inference is efficient (40-60 episodes), the training phase limits immediate deployment to domains with accurate simulators.
