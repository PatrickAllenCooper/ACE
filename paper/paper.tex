...
\begin{abstract}
Discovering causal relationships requires controlled experiments, but which experiment should come next? We propose Active Causal Experimentalist (ACE), which learns to design experiments via Direct Preference Optimization. By comparing intervention outcomes pairwise rather than estimating value functions, ACE achieves stable training under non-stationary rewards. We introduce per-node convergence criteria and dedicated root learners to handle heterogeneous mechanism learning rates. Across synthetic benchmarks, physics simulations, and economic data, ACE achieves 70--71\% improvement over baselines at equal intervention counts (p<0.001, Cohen's $d \approx 2$). Ablation experiments validate that DPO-based preference learning is essential: removing it causes 130\% degradation, reducing performance to baseline levels.
\end{abstract}
...
To validate DPO's contribution, we ablate the preference learning component by replacing it with a custom transformer policy trained via standard supervised learning (no pairwise comparisons). Across three seeds (42, 123, 456), this no-DPO variant achieves 2.12 $\pm$ 0.10 total loss, representing 130\% degradation from full ACE (0.92 $\pm$ 0.73, median 0.61). The no-DPO policy performs comparably to baseline heuristics (Random: 2.06, Round-Robin: 2.10, Max-Variance: 2.10), confirming that preference-based learning under non-stationary rewards is the critical algorithmic innovation. A custom transformer without DPO cannot discover the strategic patterns that DPO enables, validating our core methodological choice.

\subsection{Oracle Pretraining: Necessary or Accelerant?}

The DPO ablation provides strong evidence that preference-based learning, not merely having a learned policy, drives ACE's performance. The substantial degradation when removing DPO (130\%, reducing ACE to baseline-level performance) confirms this is the key algorithmic contribution.
...
ACE learns experimental design via Direct Preference Optimization, achieving 70--71\% improvement over baselines at equal episodes (171) with p<0.001 and Cohen's $d \approx 2$. Equal-episode comparisons isolate algorithmic contributions: baselines plateau at 2.06--2.10 regardless of episode count, while ACE reaches 0.61 median loss. Ablation experiments confirm DPO is essential: without preference learning, performance degrades 130\% to baseline levels (2.12). The policy autonomously discovers that concentrating interventions on collider parents is optimal, matching theoretical predictions through empirical learning.
