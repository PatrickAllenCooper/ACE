...
\begin{tikzpicture}[
    node distance=0.8cm,
    box/.style={rectangle, draw, rounded corners, minimum width=2.4cm, minimum height=0.7cm, font=\scriptsize, thick, align=center},
...
% Left column: Candidate Generation
\node[state] (state) at (0,0) {Observe State\\$s_t = (M_\theta, \{L_i\})$};
\node[process, below=of state] (gen) {Generate $K=4$\\Candidates};
\node[font=\tiny, right=0.1cm of gen] {$\{c_1, c_2, c_3, c_4\}$};

% Middle column: Evaluation
\node[process, below=of gen] (sim) {Simulate on\\Cloned Learner};
\node[font=\tiny, right=0.1cm of sim] {$\Delta\mathcal{L}(c_k)$};
\node[decision, below=of sim] (select) {Select Best\\$c^* = \argmax \Delta\mathcal{L}$};

% Right side: Execution and Update
...
% DPO branch
\node[update, right=1.5cm of sim] (dpo) {DPO Update\\$(c_{\text{best}}, c_{\text{worst}})$};
...
\draw[arrow] (sim) -- node[label, above] {preferences} (dpo);
\draw[arrow, dashed, gray] (dpo) -- ++(0,1.5) -| node[label, above, pos=0.25] {$\pi_\phi$ update} (state);
\draw[arrow, dashed, gray] (learn) -- ++(-2.5,0) |- node[label, left, pos=0.75] {next step} (state);

% Annotations
\node[font=\tiny, gray, align=left] at (-2.2,-1.2) {LLM\\policy};
\node[font=\tiny, gray, align=left] at (3.8,-2.8) {reward:\\$R = \Delta\mathcal{L} + \alpha w + \beta D$};
