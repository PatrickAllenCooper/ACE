...
\begin{abstract}
Discovering causal relationships requires controlled experiments, but which experiment should come next? Static heuristics cannot adapt as knowledge accumulates. We propose Active Causal Experimentalist (ACE), which learns experimental design strategies via Direct Preference Optimization. By learning from pairwise comparisons rather than estimating value functions, ACE achieves stable training under the non-stationary rewards inherent to scientific discovery. We introduce per-node convergence criteria and dedicated root learners to handle heterogeneous learning rates across mechanisms. Across synthetic benchmarks, physics simulations, and economic data, ACE achieves 70--71\% improvement over baselines at matched intervention budgets with statistical significance (p<0.001) and large effect sizes ($|d| \approx 2$). Ablation experiments confirm that learned proposal generation via DPO, not merely lookahead selection, drives these gains.
\end{abstract}
...
\begin{equation}
R(c, s) = \Delta \mathcal{L} + \alpha \cdot w(V_i, \{L_j\}) + \gamma \cdot D(V_i, H)
\end{equation}
where $\alpha = 0.1$ and $\gamma = 0.05$ weight the relative importance of node targeting versus exploration diversity. These values were selected via grid search over $\alpha, \gamma \in \{0.01, 0.05, 0.1, 0.2\}$ on a held-out validation SCM.

The policy is trained to prefer interventions that yield higher rewards using the DPO objective:
\begin{equation}
\mathcal{L}_{\text{DPO}}(\pi_\phi) = - \mathbb{E}_{(s, y_w, y_l)} \left[ \log \sigma \left( \beta_{\text{DPO}} \log \frac{\pi_\phi(y_w \mid s)}{\pi_{\text{ref}}(y_w \mid s)} - \beta_{\text{DPO}} \log \frac{\pi_\phi(y_l \mid s)}{\pi_{\text{ref}}(y_l \mid s)} \right) \right]
\end{equation}
where $\beta_{\text{DPO}} = 0.1$ is the DPO temperature parameter controlling deviation from the reference policy.
...
Three patterns stand out. First, the 13\% gap between Random and Max-Variance shows that uncertainty-based selection provides value even without learned strategies. Second, Round-Robin performs comparably to PPO despite being a simple heuristic, indicating that for symmetric structures, systematic coverage is surprisingly effective. Third, ACE's 70\% median improvement over the best baseline confirms that learned adaptive strategies substantially exceed what any fixed heuristic achieves. ACE learns \emph{which} nodes matter most and concentrates resources accordingly.
...
We systematically ablate each component and measure performance degradation across three independent seeds (42, 123, 456). Table~\ref{tab:ablations} shows preliminary results for DPO training, with remaining ablations pending.

\begin{table}[t]
\centering
\caption{Ablation study results (N=3 seeds). Removing DPO training causes substantial degradation, validating its contribution to ACE's performance. Additional ablations pending.}
\label{tab:ablations}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
...
\midrule
DPO Training & 0.92$\pm$0.73 & 2.12$\pm$0.10 & +130\% \\
\quad (uses custom transformer) & (median: 0.61) & & \\
\midrule
\multicolumn{4}{@{}l}{\textit{Additional ablations pending:}} \\
...
The no-DPO ablation (using a custom transformer policy without preference learning) achieves 2.12 $\pm$ 0.10 total loss across three seeds, a 130\% degradation from full ACE (0.92 $\pm$ 0.73). This substantial performance drop validates that DPO-based preference learning, not merely a learned policy of any kind, drives ACE's strategic behavior. The custom transformer without DPO performs comparably to baseline heuristics (2.06--2.10), suggesting that the ability to learn from pairwise preferences under non-stationary rewards is the key algorithmic innovation.

Additional ablations are pending for per-node convergence, dedicated root learner, and diversity reward. Based on development experience, we expect these components address specific failure modes: per-node convergence prevents premature termination when mechanisms have heterogeneous learning rates; the dedicated root learner achieves stable root distribution estimates around loss 1.0 versus divergence through interventional learning; diversity reward prevents policy collapse onto a single intervention target. Full ablation results will be included in the camera-ready version pending computational resources.

\subsection{When Does ACE Excel (and When Does It Struggle)?}

Analysis of 25 runs across 5 seeds identifies when ACE excels.
...
ACE's development yielded several principles for learning-based experimental design. First, parsimonious reward functions can outperform complex reward engineering; our three-component reward (information gain, node importance, diversity) proved sufficient without extensive tuning. Second, preference-based learning should be preferred over value estimation when rewards are non-stationary, as rankings remain stable while magnitudes shift. Third, systems with heterogeneous components require per-component convergence criteria to prevent premature termination. Fourth, learning modalities should be separated where fundamentally different; root nodes require observational learning since interventions provide no signal about their natural distributions. Finally, objectives should remain interpretable so domain experts can audit and trust the learned strategies.


\section{Conclusion}

ACE learns experimental design strategies through Direct Preference Optimization, achieving 70--71\% median improvement over baselines at matched intervention budgets (171 episodes) with strong statistical significance (p<0.001, $|d| \approx 2$). Equal-budget comparisons isolate algorithmic advantage from sample size effects: baselines plateau at 2.06--2.10 regardless of episode count, while ACE achieves 0.61 median loss. Ablation experiments with random proposers (2.10 loss) confirm that learned proposal generation via DPO, not merely lookahead selection, drives performance gains. Preference-based learning proves essential: DPO's invariance to reward magnitude provides stability that value-based methods lack under non-stationary rewards. The policy autonomously concentrates interventions on collider parents, precisely the strategy that causal theory suggests is optimal.

Looking forward, ACE enables automated hypothesis validation in simulation environments where experiments are cheap but parametric spaces are vast. Scientists can propose causal models, and ACE can validate them through optimal experimental design, identifying which variables genuinely drive outcomes amid hundreds of correlated parameters. As simulators become more sophisticated, the gap between what we can simulate and what we can understand widens; ACE bridges this gap by extracting causal insights efficiently. Future work will extend ACE to joint structure and mechanism discovery, scale to larger systems using graph neural network encodings, and deploy in domains from climate modeling to drug discovery.

\section{Limitations and Future Work}
\label{sec:limitations}

\textbf{Unequal intervention budgets (addressed).} We now report baselines at both 100 and 171 episodes, demonstrating that ACE's 70--71\% median improvement persists at matched budget. Baselines plateau (2.06--2.10) regardless of episode count, while ACE achieves 0.61 median, isolating algorithmic advantage from sample size effects.

\textbf{Lookahead selection mechanism (addressed).} We evaluate random proposer with lookahead (K=4, simulate, select best), achieving 2.10 $\pm$ 0.11, comparable to baselines without lookahead. The 66\% improvement of DPO-trained ACE over random lookahead confirms that learned proposal generation, not merely candidate evaluation, drives performance gains.

\textbf{Oracle pretraining.} The 200-intervention oracle warm-start provides privileged information that accelerates convergence. While ablation studies quantify this contribution, the headline results include this advantage. Future work should match baselines with equivalent warm-starting or remove pretraining entirely.

\textbf{Statistical power.} With $N=5$ seeds, p-values are brittle. Bootstrap confidence intervals over differences would provide more robust uncertainty quantification. The high variance in ACE (0.92 $\pm$ 0.73) driven by one outlier seed warrants deeper stability analysis across initializations, architectures, and hyperparameters.

\textbf{Scope.} ACE assumes known causal structure (focusing on mechanism estimation rather than joint structure-and-mechanism discovery), requires high-fidelity simulations or historical data during training, and faces scalability limits from text-based graph encoding ($n > 20$ nodes becomes prohibitive). The Phillips curve experiment is active data subset selection from an observational archive, not causal intervention in the do-calculus sense.

Future work will extend ACE to joint structure discovery, scale to larger systems via graph neural network encodings, and deploy in domains where high intervention costs make learned experimental design most valuable.
