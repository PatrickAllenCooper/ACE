...
Across 5 independent runs (N=5), Duffing oscillator experiments achieve final coupling error of 0.042 $\pm$ 0.036 (95\% CI: [0.011, 0.073]) after 100 episodes. All runs successfully recovered the true chain topology from initially apparent full connectivity. Interventions on intermediate oscillators ($X_2$) decouple the synchronized system, breaking spurious $X_1$--$X_3$ correlations, validating that strategic intervention on mediating variables reveals true causal structure hidden by synchronization dynamics.
...
Phillips curve experiments demonstrate ACE's ability to select informative historical periods for mechanism learning. Five independent runs (N=5) validate retrospective causal discovery on real-world economic data, with systematic querying of high-volatility historical regimes (1970s stagflation, Great Recession) exposing mechanism nonlinearities. Early exposure to structural breaks improves generalization to held-out data, demonstrating the value of strategic historical sampling for retrospective causal learning.
...
ACE excels when causal structures contain colliders or complex dependencies requiring strategic intervention allocation (99.8\% concentration on collider parents, $L_{X_3} = 0.054$ vs baseline $\sim$2.0+), when experimental budgets allow policy learning (171 episodes on average), when mechanisms have heterogeneous learning rates benefiting from per-node convergence (linear $X_2$: 0.010, quadratic $X_5$: 0.449, collider $X_3$: 0.054), and when structural knowledge exists (known or hypothesized graph).

However, one seed (789) exhibited $X_5$ mechanism failure (loss 1.73 vs 0.02--0.22 for other seeds), suggesting sensitivity to initialization or optimization challenges in quadratic mechanisms. This outlier motivates robust reporting using median statistics and highlights the value of multi-seed validation.
