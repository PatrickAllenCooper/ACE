...
\textbf{Random} sampling provides an unguided exploration baseline, selecting both the target node $V_i$ and intervention value $x \in [-5,5]$ uniformly at random. This represents what an experimentalist would achieve without any strategic guidance.

\textbf{Round-Robin} ensures systematic coverage by cycling through nodes in a fixed order $V_{t \pmod n}$. This simple heuristic guarantees that no mechanism is neglected, providing a strong baseline for symmetric structures where all nodes are equally important.

\textbf{Max-Variance} implements uncertainty sampling, selecting the intervention that maximizes predicted outcome variance: $c^* = \arg\max_c \sum_{V_j} \text{Var}_{M_\theta}[V_j \mid c]$. Variance estimates are obtained via Monte Carlo Dropout, targeting regions where the learner is most uncertain.

\textbf{PPO} \cite{schulman2017proximal} provides a learned baseline using value-based reinforcement learning. We implement an actor-critic architecture with identical reward shaping to ACE, using Generalized Advantage Estimation ($\lambda=0.95$), clipped surrogate objective ($\epsilon=0.2$), and entropy regularization. This comparison isolates DPO's algorithmic contribution from differences in reward design.
...
We first establish baseline performance through 5 independent runs (seeds: 42, 123, 456, 789, 1011). After 100 episodes of training, the baselines achieve the following total MSE: Random 2.18 $\pm$ 0.06 (95\% CI: [2.06, 2.31]), Round-Robin 2.15 $\pm$ 0.08 (95\% CI: [2.00, 2.30]), Max-Variance 2.05 $\pm$ 0.12 (95\% CI: [1.80, 2.29]), and PPO 2.11 $\pm$ 0.13 (95\% CI: [2.00, 2.23]). Among these, Max-Variance achieves the best performance, demonstrating that uncertainty-guided sampling provides meaningful advantages for mechanism learning even without learned strategies. 

ACE dramatically outperforms all baselines, achieving 0.61 median total MSE (mean: 0.92 $\pm$ 0.73, N=5) over an average of 171 episodes. This represents 52--58\% improvement across the board. We report median for robustness, as one seed exhibited a mechanism failure on $X_5$ that inflates the mean; even so, the mean improvement remains substantial. Against the strongest baseline (Max-Variance), ACE demonstrates 68\% improvement with 95\% CI [0.02, 1.82].

Independent samples t-tests with Bonferroni correction ($\alpha = 0.0125$) confirm statistical significance for three of four baselines: PPO (p=0.0046, Cohen's d=-2.46), Random (p=0.0063, d=-2.32), and Round-Robin (p=0.0092, d=-2.16). The comparison to Max-Variance yields large practical improvement (52\%, d=-1.96) with marginal statistical significance (p=0.0146 vs threshold 0.0125). Per-node analysis reveals particularly striking results for the collider mechanism: ACE achieves $L_{X_3} = 0.054 \pm 0.014$, validating that its strategic intervention allocation successfully targets the most challenging structure in the graph.
...
Across all 25 experimental runs (5 methods $\times$ 5 seeds), a clear performance hierarchy emerges. ACE achieves a median loss of 0.61 (mean: 0.92 $\pm$ 0.73), representing 52--58\% improvement over every baseline. Among the baselines themselves, Max-Variance achieves the lowest error (1.93 $\pm$ 0.04), followed by Round-Robin (2.03 $\pm$ 0.05), Random (2.11 $\pm$ 0.05), and PPO (2.19 $\pm$ 0.07).

Statistical testing with Bonferroni correction ($\alpha = 0.0125$) confirms that ACE significantly outperforms three of four baselines: PPO (p=0.0046), Random (p=0.0063), and Round-Robin (p=0.0092), all with large effect sizes (Cohen's d $\approx$ 2.2). The comparison to Max-Variance shows substantial practical improvement (52\%, d=-1.96) but falls just short of statistical significance after correction (p=0.0146), reflecting Max-Variance's effectiveness as an uncertainty-guided heuristic.

These results reveal interesting patterns in baseline performance. The 13\% gap between Random and Max-Variance demonstrates the value of uncertainty-based selection even without learning. Meanwhile, the simple Round-Robin heuristic (2.15) performs comparably to the learned PPO policy (2.11), suggesting that for symmetric structures, systematic coverage provides a surprisingly strong baseline. ACE's additional 70\% improvement over Max-Variance (using median) validates that learned adaptive strategies can substantially exceed what heuristics achieve.
...
Looking forward, ACE enables a new paradigm of automated hypothesis validation. Scientists can propose causal models, and ACE can rapidly validate them through optimal experimental design in simulators or against historical data. This human-AI collaboration has the potential to compress discovery cycles from months to hours in domains with mature computational infrastructure. Future work will extend ACE to joint structure and mechanism discovery, scale to larger systems using graph neural network encodings, and deploy in domains where high intervention costs make sample efficiency critical.

\section{Limitations and Future Work}

Several limitations of ACE warrant discussion. First, the framework requires high-fidelity simulations during policy training, limiting immediate application to domains that possess accurate simulators or sufficient historical data to serve as oracles. Domains like drug discovery or social science, where ground truth is expensive or impossible to obtain, would require additional methodological development.

Second, our current implementation assumes known causal structure, focusing on the mechanism estimation problem rather than joint structure-and-mechanism discovery. While this assumption holds in many practical settings where domain expertise provides structural hypotheses, extending ACE to simultaneously discover structure remains an important direction.

Third, our text-based graph encoding faces context length limitations as system size grows. For graphs with $n > 20$ nodes, the prompt length becomes prohibitive. Graph neural network encodings could dramatically improve scalability, enabling application to domains like gene regulatory networks where hundreds of variables interact.

Finally, while ACE achieves superior final performance (52--58\% improvement), it does not reduce the total number of episodes compared to fixed-episode baselines (171 vs 100 episodes on average). This reflects a quality-versus-quantity tradeoff: ACE invests additional episodes to achieve substantially better mechanism estimates. This tradeoff favors applications where final accuracy is more critical than minimizing experiment count---common in domains where each experiment provides rich information but computational rather than monetary cost dominates.
