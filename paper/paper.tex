...
\begin{figure}[t]
\centering
% INSERT: \includegraphics[width=0.8\columnwidth]{figures/synthetic_scm.pdf}
% DIAGRAM: Two disconnected subgraphs
% Left: X1 -> X2 -> X3, X1 -> X3 (collider at X3)
% Right: X4 -> X5
% Label edges with mechanism types: linear, nonlinear, quadratic
\fbox{\parbox{0.75\columnwidth}{\centering\vspace{2em}[Synthetic 5-Node SCM Diagram]\vspace{2em}}}
\caption{Structure of the synthetic 5-node benchmark SCM. $X_3$ is a collider receiving edges from both $X_1$ and $X_2$. The disconnected subgraph $X_4 \to X_5$ tests quadratic mechanism learning.}
...
% INSERT: \input{figures/synthetic_learning.tex}
...
% INSERT: \input{figures/intervention_distribution.tex}
...
\begin{figure}[t]
\centering
% INSERT: \includegraphics[width=\columnwidth]{figures/complex_scm.pdf}
% DIAGRAM: 15-node graph with 5 colliders highlighted
% Show nested collider structure, mixed mechanism types
% Color-code: roots (blue), colliders (red), other (gray)
\fbox{\parbox{0.9\columnwidth}{\centering\vspace{3em}[Complex 15-Node SCM Diagram]\vspace{3em}}}
\caption{Structure of the complex 15-node SCM. Collider nodes (red) require interventions on multiple parents for identification. Nested structures test the policy's ability to reason about causal depth.}
...
The strategic advantage of ACE becomes more pronounced at scale: while random sampling achieves [RANDOM\_COLLIDER] MSE on colliders, ACE achieves [ACE\_COLLIDER], a [IMPROVEMENT]\% improvement. On non-collider nodes, performance is comparable ([ACE\_NONCOLLIDER] vs. [RANDOM\_NONCOLLIDER]), confirming that ACE's gains come from intelligent prioritization rather than uniform improvement.
...
\begin{figure}[t]
\centering
% INSERT: \includegraphics[width=0.9\columnwidth]{figures/duffing_scm.pdf}
% DIAGRAM: Two panels
% Left: True structure (chain X1 -- X2 -- X3 with spring symbols)
% Right: Apparent structure (fully connected due to synchronization)
% Show how clamping X2 breaks spurious correlations
\fbox{\parbox{0.85\columnwidth}{\centering\vspace{2em}[Duffing Oscillator Coupling Diagram]\vspace{2em}}}
\caption{Coupled Duffing oscillators. Left: true chain coupling. Right: apparent full connectivity due to synchronization. ACE discovers that clamping $X_2$ breaks spurious correlations.}
...
ACE discovers a ``clamping'' strategy: by intervening to hold the middle oscillator fixed ($\text{do}(X_2 = 0)$), it breaks the correlation chain and reveals the true coupling structure. This emergent strategy achieves [STRUCTURE\_F1] F1 score for structure identification compared to [BASELINE\_F1] for Max-Variance.
...
\begin{figure}[t]
\centering
% INSERT: \includegraphics[width=0.85\columnwidth]{figures/phillips_scm.pdf}
% DIAGRAM: Economic SCM with temporal structure
% UNRATE, FEDFUNDS, MICH all pointing to CPI_{t+1}
% Annotate with regime periods (1970s, 1990s, 2008, etc.)
\fbox{\parbox{0.8\columnwidth}{\centering\vspace{2em}[Phillips Curve SCM Diagram]\vspace{2em}}}
\caption{Phillips curve causal structure. Unemployment, federal funds rate, and inflation expectations jointly determine future CPI. Historical regimes provide natural variation for mechanism identification.}
...
ACE learns to query high-volatility regimes (1970s stagflation, 2008 crisis) that expose mechanism nonlinearities, achieving [OOS\_MSE] out-of-sample MSE compared to [BASELINE\_OOS] for chronological sampling. Notably, ACE allocates [HIGH\_VOL\_PCT]\% of queries to pre-1985 data despite this representing only [PRE1985\_PCT]\% of available observations, demonstrating adaptive regime selection.
