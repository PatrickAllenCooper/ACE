...
\begin{equation}
\mathcal{L}_{\text{DPO}} = - \mathbb{E} \left[ \log \text{sigmoid} \left( \beta \left[ \log \frac{\pi_\phi(y_w)}{\pi_{\text{ref}}(y_w)} - \log \frac{\pi_\phi(y_l)}{\pi_{\text{ref}}(y_l)} \right] \right) \right]
\end{equation}
...
\begin{tikzpicture}[
    scale=0.85,
    node distance=1.8cm,
    root/.style={circle, draw, fill=blue!20, minimum size=0.75cm, font=\small, thick},
    regular/.style={circle, draw, fill=gray!15, minimum size=0.75cm, font=\small, thick},
    collider/.style={circle, draw, fill=red!25, minimum size=0.75cm, font=\small, thick},
    arrow/.style={-{Stealth[length=2.5mm]}, thick}
]
...
\node[regular] (X2) at (0,-2) {$X_2$};
\node[regular] (X5) at (3.5,-2) {$X_5$};

% Layer 2: Collider
\node[collider] (X3) at (0,-4) {$X_3$};
