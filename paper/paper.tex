...
We validate ACE's architectural components through ablation studies. Table~\ref{tab:ablations} shows that removing the diversity reward causes substantial performance degradation (206\%, reducing ACE to baseline-level performance). This confirms that exploration diversity is essential for ACE's strategic behavior.

\begin{table}[t]
\centering
\caption{Ablation study results (N=2 seeds, 100 episodes). Removing diversity reward causes substantial degradation to near-baseline levels. Additional ablations pending.}
\label{tab:ablations}
\footnotesize
\begin{tabular}{@{}lcc@{}}
\toprule
...
\midrule
Diversity Reward & 2.82$\pm$0.22 & +206\% \\
...
The diversity ablation demonstrates that without exploration mechanisms, ACE degrades to baseline performance (2.82 vs 2.03--2.10 baselines). This validates that strategic exploration, not just DPO-based proposal generation, is critical for ACE's advantage. The removal of diversity reward causes the policy to over-concentrate interventions, preventing effective mechanism learning across all nodes.
