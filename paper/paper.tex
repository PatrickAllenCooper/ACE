...
\subsection{Reinforcement Learning for Experimental Design}

Recent work has begun exploring reinforcement learning for scientific discovery tasks. \cite{zhou2021optimizing} applied RL to Bayesian optimization for materials discovery, while \cite{roch2020bandits} used multi-armed bandits for adaptive clinical trial design. However, these applications focus on optimization (finding best parameters) rather than causal structure discovery, where the goal is learning relationships rather than outcomes.

Critically, standard RL approaches face a fundamental challenge in our setting: non-stationarity. As the learner improves its causal model, the value of different interventions changes---what was informative early on may become redundant later. Value-based RL methods (Q-learning, actor-critic) struggle with such shifting rewards. This motivates our use of preference-based learning (DPO), which compares interventions pairwise rather than assigning absolute values, providing more stable training signals when optimal strategies evolve during learning.
...
A particularly exciting direction is \emph{automated hypothesis validation}: experimental scientists could propose candidate SCMs based on domain intuition, and ACE could immediately verify or reject them through interaction with high-fidelity simulators or sufficiently robust historical datasets. Consider a biologist proposing "I think protein X regulates gene Y which affects phenotype Z"---ACE could rapidly test this hypothesis by designing optimal interventions in a cellular simulator, returning within hours whether the proposed mechanism is consistent with simulated data. Similarly, an engineer proposing a causal model of factory defects could validate it against years of process logs, with ACE automatically designing the diagnostic experiments needed to confirm or refute each hypothesized causal link.

This creates a vision of \emph{human-AI collaborative discovery}: domain experts propose hypotheses leveraging their intuition and contextual knowledge, while ACE handles the combinatorially complex task of experimental validation. The scientist provides the "what if" (candidate mechanisms), and the system provides rapid, data-driven "yes/no" verdicts through optimal experimental design. In domains with mature simulators (computational chemistry, circuit design, climate modeling) or extensive historical data (electronic health records, industrial process logs, financial markets), this could dramatically accelerate the hypothesis → experiment → validation cycle from months to hours.

The framework's ability to leverage pretrained language models further suggests that scientists could specify hypotheses in natural language ("does inflation expectation Granger-cause CPI?"), which ACE translates to formal SCMs, tests through optimized interventions, and reports back with evidence-based verdicts. This human-in-the-loop paradigm preserves scientific creativity and domain expertise while automating the laborious experimental validation process.
