...
\begin{abstract}
Discovering causal relationships requires controlled experiments, but which experiment should come next? We propose Active Causal Experimentalist (ACE), which learns to design experiments via Direct Preference Optimization. By comparing intervention outcomes pairwise rather than estimating value functions, ACE achieves stable training under non-stationary rewards. We introduce per-node convergence criteria and dedicated root learners to handle heterogeneous mechanism learning rates. Across synthetic benchmarks, physics simulations, and economic data, ACE achieves 70--71\% improvement over baselines at equal intervention counts (p<0.001, Cohen's $d \approx 2$). Comprehensive ablation studies validate all four architectural components: removing any causes 130--156\% degradation, reducing performance to baseline levels.
\end{abstract}
...
ACE learns experimental design via Direct Preference Optimization, achieving 70--71\% improvement over baselines at equal episodes (171) with p<0.001 and Cohen's $d \approx 2$. Equal-episode comparisons isolate algorithmic contributions: baselines plateau at 2.06--2.10 regardless of episode count, while ACE reaches 0.61 median loss. Comprehensive ablation studies validate all four architectural components, with degradation ranging from 130--156\% when any is removed, confirming that each contributes substantially to ACE's performance. The policy autonomously discovers that concentrating interventions on collider parents is optimal, matching theoretical predictions through empirical learning.
