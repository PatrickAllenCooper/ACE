...
\begin{table}[t]
\centering
\caption{Main results on the 5-node synthetic benchmark (N=5 seeds). ACE outperforms all baselines; $^{**}$p<0.0125 (Bonferroni corrected). Note: ACE uses 171 episodes on average vs 100 for baselines (see text).}
...
ACE dramatically outperforms all methods, achieving 0.61 median total MSE (mean: 0.92 $\pm$ 0.73). We report median for robustness, as one seed (789) exhibited a mechanism failure on $X_5$ that inflates the mean; this outlier achieved loss 1.73 on $X_5$ compared to 0.02--0.22 for other seeds, suggesting sensitivity to initialization that warrants further investigation. Paired t-tests with Bonferroni correction ($\alpha = 0.0125$) using matched seeds confirm statistical significance for PPO (p=0.0046), Random (p=0.0063), and Round-Robin (p=0.0092). The comparison to Max-Variance shows substantial practical improvement (52\%, $|d|$=1.96) but marginal significance (p=0.0146 $>$ 0.0125). Per-node analysis reveals particularly striking collider learning: $L_{X_3} = 0.054 \pm 0.014$, validating ACE's strategic intervention allocation.
...
Each component plays a distinct role. Per-node convergence prevents premature termination: in our experiments, one seed achieved successful early stopping at 60 episodes while others required up to 199, demonstrating the importance of mechanism-specific stopping criteria. The dedicated root learner proves essential for exogenous variables, achieving root losses around 1.0 compared to divergence when roots are learned through the standard interventional pathway. The diversity reward prevents policy collapse by encouraging exploration across intervention values; without it, the policy might over-focus on a single node, whereas with diversity regularization it maintains 99.8\% strategic concentration on collider parents while still exploring sufficient value ranges. Each component addresses a specific failure mode we observed during development.
...
ACE learns experimental design strategies through Direct Preference Optimization, achieving 52--58\% improvement over baselines with statistical significance for three of four comparisons ($|d| \approx 2$). Preference-based learning proves essential: DPO's invariance to reward magnitude provides stability that value-based methods lack under the non-stationary rewards of scientific discovery. Per-node convergence criteria and dedicated root learners address the heterogeneous learning rates across mechanism types. The policy autonomously concentrates interventions on collider parents, precisely the strategy that causal theory suggests is optimal, mirroring how skilled experimentalists adapt their strategies as understanding grows.
...
\textbf{Unequal intervention budgets.} ACE uses more episodes than fixed baselines (171 vs 100 on average), making direct comparison of final accuracy potentially confounded by sample size. While we frame this as a quality-over-quantity tradeoff suited to domains where accuracy matters more than sample count, future work should report learning curves and area-under-curve metrics to enable fair sample-efficiency comparisons. Equal-budget evaluations (ACE at 100 episodes) would isolate the algorithmic contribution from the budget effect.

\textbf{Lookahead selection mechanism.} ACE's candidate evaluation step (simulate $K$ candidates on cloned learner, select best) constitutes 1-step planning with an internal evaluator. This lookahead mechanism itself provides value beyond pure policy learning. A critical missing baseline is the same lookahead selector with non-learned proposers (e.g., random candidates, Sobol sequences, Bayesian optimization over $\nu$). Without this ablation, some gains may derive from the evaluation-and-select mechanism rather than DPO training. We plan to address this in future work.
...
Future work will extend ACE to joint structure discovery, add equal-budget comparisons and ablations of the lookahead mechanism, scale to larger systems via graph neural network encodings, and deploy in domains where high intervention costs make learned experimental design most valuable.
