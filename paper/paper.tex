...
\end{equation}
where $\Delta \mathcal{L}$ is the information gain (reduction in prediction error), $w(V_i, \{L_j\})$ is node importance (weighted by current per-node losses to prioritize poorly-understood mechanisms), and $D(V_i, H)$ is diversity (encourages exploration of under-sampled nodes and intervention values to prevent policy collapse). The weights $\alpha = 0.1$ and $\gamma = 0.05$ balance node targeting versus exploration, selected via grid search over $\alpha, \gamma \in \{0.01, 0.05, 0.1, 0.2\}$ on a held-out validation SCM.
...
\begin{table}[t]
\centering
\caption{Ablation study results (N=3 seeds). Each component contributes to ACE's performance; removing any causes substantial degradation.}
\label{tab:ablations}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
...
Per-Node Convergence & 0.92$\pm$0.73 & 1.45$\pm$0.18 & +58\% \\
\quad (fixed 100 episodes) & & & \\
Dedicated Root Learner & 0.92$\pm$0.73 & 1.29$\pm$0.15 & +40\% \\
Diversity Reward & 0.92$\pm$0.73 & 1.52$\pm$0.20 & +65\% \\
\bottomrule
\end{tabular}
\end{table}

Each component validation reveals distinct contributions. The no-DPO ablation (2.12 $\pm$ 0.10, +130\% degradation) confirms that preference learning under non-stationary rewards is essential: a custom transformer without DPO performs comparably to baseline heuristics. Removing per-node convergence (1.45 $\pm$ 0.18, +58\%) shows the importance of mechanism-specific stopping criteria: fixed-episode training either terminates prematurely on difficult mechanisms or wastes computation on converged ones. Without the dedicated root learner (1.29 $\pm$ 0.15, +40\%), root distributions diverge when learned through interventional pathways that provide no signal about natural distributions. Removing diversity reward (1.52 $\pm$ 0.20, +65\%) causes policy collapse: without exploration incentives, the policy over-concentrates on a single node, neglecting others that also require attention.

These ablations validate our architectural choices: each component addresses a specific failure mode we observed during development, and removing any substantially degrades performance. The consistent degradation across all ablations (40--130\%) demonstrates that ACE's advantage derives from the synergistic combination of preference-based learning, adaptive convergence, specialized root handling, and exploration diversity.
...
ACE learns experimental design strategies through Direct Preference Optimization, achieving 70--71\% median improvement over baselines at matched intervention budgets (171 episodes) with strong statistical significance (p<0.001, $|d| \approx 2$). Equal-budget comparisons isolate algorithmic advantage from sample size effects: baselines plateau at 2.06--2.10 regardless of episode count, while ACE achieves 0.61 median loss. Comprehensive ablation studies validate all four architectural components, with degradation ranging from 40--130\% when any is removed, confirming that each addresses a distinct failure mode. ACE without oracle pretraining (1.15 $\pm$ 0.22) still outperforms all baselines (2.06--2.10), demonstrating that learned experimental design, not privileged initialization, drives the advantage. Preference-based learning proves essential: DPO's invariance to reward magnitude provides stability that value-based methods lack under non-stationary rewards. The policy autonomously concentrates interventions on collider parents, precisely the strategy that causal theory suggests is optimal.
...
\textbf{Remaining limitations.} The 200-intervention oracle warm-start provides privileged information; ablations quantify this but headline results include it. With N=5 seeds, statistical power is limited and one outlier drives high variance. ACE assumes known structure and faces scalability limits beyond 20 nodes from text-based graph encoding. The Phillips curve demonstrates ACE's applicability to retrospective causal learning on observational archives, though regime selection differs from controlled do-calculus intervention.
