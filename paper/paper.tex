...
\begin{tikzpicture}[
    ...
    collider/.style={circle, draw, thick, minimum size=0.7cm, font=\small, fill=red!15},
    arrow/.style={-{Stealth[length=2mm]}, thick},
    edge_label/.style={font=\tiny, above, midway}
]

% Collider subgraph
\node[node] (X1) at (0,0) {$X_1$};
\node[node] (X2) at (2,0) {$X_2$};
\node[collider] (X3) at (4,0) {$X_3$};

\draw[arrow] (X1) -- node[edge_label] {lin} (X2);
\draw[arrow] (X2) -- node[edge_label] {nonlin} (X3);
\draw[arrow] (X1) to[bend left=25] (X3);

% Separate subgraph
\node[node] (X4) at (5.5,0) {$X_4$};
\node[node] (X5) at (7.5,0) {$X_5$};

\draw[arrow] (X4) -- node[edge_label] {quad} (X5);

% Annotation
\node[below=0.25cm of X3, font=\tiny, text=red!70!black] {collider};
