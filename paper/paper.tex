...
\node[node] (X4) at (2.5,0) {$X_4$};

% Layer 1
\node[node] (X2) at (0,-1.5) {$X_2$};
\node[node] (X5) at (2.5,-1.5) {$X_5$};
...
\draw[arrow] (X1) -- (X2);
\draw[arrow] (X2) -- (X3);
\draw[arrow] (X1) to[bend right=25] (X3);
\draw[arrow] (X4) -- (X5);

% Annotation
\node[right=0.1cm of X3, font=\scriptsize, text=red!70!black] {\textit{collider}};
