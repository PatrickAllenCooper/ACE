...
\begin{abstract}
Understanding cause-and-effect relationships in complex systems requires running controlled experiments. In \emph{causal inference}---the study of how variables influence each other through directed causal pathways---we must decide which variables to actively manipulate (intervene upon) and what values to set them to. This challenge of \emph{optimal experimental design}---selecting interventions that most efficiently reveal causal structure---is central to scientific discovery, clinical trials, and industrial process optimization.

Existing approaches rely on static heuristics or greedy optimization that fail to adapt as our knowledge evolves. We propose \textit{Active Causal Experimentalist} (ACE), a framework that learns experimental design strategies through self-play using Direct Preference Optimization (DPO). ACE trains a policy to propose interventions by comparing their information gain for a learner attempting to recover structural causal model (SCM) mechanisms---the mathematical equations describing how each variable depends on its causes. Unlike value-based reinforcement learning, which struggles with non-stationary reward signals as the learner improves, DPO's preference-based objective remains stable throughout training.

We introduce per-node convergence criteria for intelligent early stopping and a dedicated root learner to handle exogenous (external cause) variables. On synthetic benchmarks, ACE achieves competitive performance with uncertainty sampling baselines while demonstrating superior identification of colliders (variables with multiple causes). We validate the approach on physics simulations (coupled Duffing oscillators) and economic data (Phillips curve), showing that learned experimental strategies can match or exceed domain-agnostic heuristics, achieving an 80\% reduction in experimental budget.
\end{abstract}

\section{Introduction}

Motivation. Every experimentalist faces the same fundamental constraint: limited resources to explore an enormous space of possibilities. A molecular biologist perturbing gene expression must choose from thousands of potential knockdowns, each requiring weeks of cell culture. A materials scientist optimizing alloy composition confronts an exponential number of element combinations and processing conditions. A neuroscientist using optogenetics can stimulate individual neurons but faces strict limits on experiment duration to maintain tissue viability. In each case, the experimentalist must answer: \emph{which experiment should I run next?}

The traditional answer---domain expertise, intuition, or exhaustive search---breaks down as systems grow complex. Consider drug discovery: testing all pairwise combinations of 100 candidate compounds requires 4,950 experiments; adding dosage levels multiplies this further. A materials scientist exploring a 10-component alloy across 5 processing temperatures faces $5^{10}$ possible configurations. These combinatorial explosions force experimentalists to adopt principled strategies for intervention selection.

The Causal Discovery Challenge. Scientific discovery fundamentally relies on understanding \emph{causal} relationships---determining not just that variables correlate, but that one influences another through a directed mechanistic pathway. In \emph{causal inference}---the formal study of cause-and-effect relationships---we must move beyond passive observation to actively manipulate variables through \emph{interventions}: setting a variable to a controlled value to isolate its causal effects. This is the scientific method in action: varying one factor while controlling others to establish causation.

The efficiency of learning these causal relationships depends critically on \emph{which} variables we choose to intervene upon and \emph{what values} we set them to. This challenge of \emph{optimal experimental design}---selecting the sequence of interventions that most efficiently reveals the complete causal structure---is universal across experimental science. While theoretical results establish bounds on the number of interventions required for identifiability \cite{eberhardt2005number,eberhardt2006n}, these worst-case guarantees provide limited guidance for the adaptive, sequential decision-making that characterizes real experimental practice.

Limitations of Current Approaches. Traditional methods for experimental design employ static heuristics---random sampling, round-robin coverage, or greedy information maximization \cite{murphy2001active,hauser2012characterization}. A chemist using random sampling might test reactant concentrations uniformly across a range, wasting experiments on uninteresting regions. Round-robin strategies ensure coverage but ignore accumulating evidence about which variables matter most. Greedy uncertainty sampling, while locally optimal, can miss global structure: in a metabolic pathway, perturbing the rate-limiting enzyme might seem uncertain but provides little information about upstream regulation.

These methods share two critical limitations: they treat each causal system in isolation, unable to transfer insights from prior experimental campaigns, and they optimize single objectives without balancing the multiple considerations experimentalists face in practice. A biologist may care not only about identifying the causal graph but also about minimizing reagent costs (which vary by orders of magnitude), completing experiments before grant renewal, and obtaining publishable-quality mechanism estimates---not just graph topology. Current methods cannot adapt to these multi-objective, context-dependent constraints.

Our Approach. We present \textit{Active Causal Experimentalist} (ACE), a framework that formulates experimental design as a learnable sequential decision problem, mimicking how skilled experimentalists develop intuition over their careers. The key insight is to model the scientific process as an \emph{iterative cycle}: an \textit{experimentalist} proposes interventions, a \textit{learner} updates beliefs about mechanisms from the results, and the experimentalist adapts its strategy based on what was learned. Each experiment informs the next, creating a virtuous cycle of discovery.

This mirrors real experimental practice. A biologist doesn't plan all 100 experiments upfront---they run experiment 1, analyze results, notice that perturbing gene A revealed unexpected interactions, then design experiment 2 to investigate those interactions. ACE formalizes this iterative process: after each intervention, the learner's updated state (which mechanisms are now understood, which remain uncertain) guides the policy's next proposal. This closed-loop interaction allows strategies to adapt as knowledge accumulates.

Crucially, ACE learns from the \emph{outcomes} of experiments: did intervening on gene A provide more information than gene B? Did testing reactant concentration C help more than temperature T? By training via Direct Preference Optimization (DPO) \cite{rafailov2023direct} on such pairwise comparisons, ACE learns adaptive strategies without requiring explicit value function estimation---a critical advantage given the non-stationary nature of scientific discovery (the "best" next experiment changes as our knowledge grows).

The result is a system that can assist experimentalists in practice: suggesting which experiment to run next based on current knowledge, learning from outcomes to improve future suggestions, and adapting to domain-specific constraints like intervention costs or time limits. This virtuous cycle---experiment → learn → adapt strategy → better experiment→ better learning---captures how experimental intuition develops and provides data-driven guidance that improves with each iteration.

Our contributions are threefold: (1) We introduce a parsimonious three-component reward formulation that balances information gain, node importance, and exploration diversity, providing interpretable guidance without complex reward engineering; (2) We propose per-node convergence criteria and dedicated root learners to address the heterogeneous learning rates of exogenous (root cause) and endogenous (effect) variables in causal systems; (3) We demonstrate on synthetic and real-world domains that preference-based learning provides a stable alternative to value-based reinforcement learning for experimental design, achieving 80\% computational savings while maintaining competitive performance in discovering causal structure.
...
We formulate causal experimental design as a sequential decision problem where a policy learns to select interventions by observing their effect on a learner's epistemic state. Critically, the policy must learn \emph{both} which variable to intervene upon (target selection) \emph{and} what value to set it to (functional intervention approximation)---a joint action space that distinguishes our approach from methods that only address target selection.
...
At each step $t$, the interaction proceeds as an \emph{iterative discovery cycle}:
\begin{enumerate}
...
This creates a \emph{virtuous cycle}: better understanding $\to$ more informed interventions $\to$ more informative data $\to$ better understanding. The cycle continues until per-node convergence criteria detect that all mechanisms have been learned, at which point the discovery process naturally terminates.
...
\emph{Statistical Validation Protocol.}
...
\emph{Intervention Distribution.} Figure~\ref{fig:intervention-dist} visualizes the learned intervention policy. ACE concentrates interventions on $X_2$ and $X_1$ (the collider's parents) \textcolor{red}{[TODO: Report X₁+X₂ percentage > 60\% from metrics.csv to verify Line 485 strategic concentration claim]}, compared to approximately 40\% uniform allocation (20\% each) under random sampling. This strategic allocation explains the improved collider performance.
...
% TODO: Add Baseline Comparison Figure
% \begin{figure}[t]
% \centering
...
\emph{Why Preference Learning Works for Experimental Design.} The theoretical justification stems from the Bradley-Terry model underlying DPO: preference probabilities $P(y_w \succ y_l \mid s)$ depend only on reward \emph{differences}, not absolute magnitudes. When rewards are non-stationary (shifting over time), differences can remain stable even as individual rewards change. Formally, if $r_t(a) = f(t) \cdot r_0(a)$ where $f(t)$ is a time-varying scale factor, the preference $P(a \succ b)$ depends on $r_0(a) - r_0(b)$, which is invariant to $f(t)$. This scale-invariance property provides robustness to the inherent non-stationarity of experimental design, where information gain decays as $\mathcal{O}(1/t)$ with accumulated evidence \cite{murphy2001active}.
...
\begin{enumerate}
\item Prefer rankings over magnitudes when reward signals are non-stationary---a property shared by scientific discovery (diminishing information gain) and industrial optimization (diminishing defect rates)
\item Respect heterogeneous timescales with per-component convergence criteria, essential when systems contain both fast-responding variables (e.g., temperature) and slow-responding ones (e.g., material fatigue)
\item Isolate observational learning for components where interventions provide no signal---in manufacturing, this corresponds to environmental factors that cannot be controlled but must be modeled
\item Simplify reward functions to interpretable, low-dimensional objectives that domain experts can audit and trust
\end{enumerate}
...
Critically, the iterative nature of our approach---where each experiment's outcome immediately informs the next intervention proposal---creates a virtuous cycle of discovery that mirrors how skilled experimentalists work. Rather than planning all experiments upfront, the system adapts its strategy as understanding grows, concentrating effort where uncertainty remains and terminating when mechanisms converge. This closed-loop interaction between experimentation and learning is fundamental to efficient scientific discovery, yet rarely formalized in experimental design methods.

Looking forward, this virtuous cycle enables a paradigm of \emph{automated hypothesis validation}: scientists propose candidate causal models based on domain intuition, and ACE rapidly validates them through optimal experimental design in simulators or against historical data. This human-AI collaboration---where scientists provide hypotheses and the system provides rapid experimental verdicts---could compress the discovery cycle from months to hours in domains with mature computational infrastructure.
...
A particularly exciting direction is \emph{automated hypothesis validation}: experimental scientists could propose candidate SCMs (specifying the graph structure and hypothesized mechanisms) based on domain intuition, and ACE could immediately verify or reject them through interaction with high-fidelity simulators or sufficiently robust historical datasets. Consider a biologist proposing a specific causal graph for gene regulation (e.g., "X₁ → X₂ → X₃ with X₁ also directly affecting X₃")---ACE could rapidly test this structure by designing optimal interventions in a cellular simulator, returning within hours whether the proposed graph is consistent with simulated data and estimating the mechanism parameters. Similarly, an engineer proposing a causal model of factory defects (specifying which process variables affect which outcomes) could validate it against years of process logs, with ACE automatically designing the diagnostic experiments needed to confirm or refute each hypothesized causal link.

This creates a vision of \emph{human-AI collaborative discovery}: domain experts propose hypotheses as explicit causal graphs leveraging their intuition and contextual knowledge, while ACE handles the combinatorially complex task of experimental validation through optimal intervention design. The scientist provides the "what if" (candidate graph structures), and the system provides rapid, data-driven verdicts on mechanism consistency through efficient experimentation. In domains with mature simulators (computational chemistry, circuit design, climate modeling) or extensive historical data (electronic health records, industrial process logs, financial markets), this could dramatically accelerate the hypothesis → experiment → validation cycle from months to hours.

Note that while our implementation uses LLMs to generate intervention proposals in text form ("do(X₂ = 1.5)"), the scientist must still provide the causal graph structure explicitly. The natural language interface introduces a source of ambiguity (parsing, validation) that requires careful prompt engineering and error handling---advantages of explicit symbolic representations. Future work could explore tighter integration of domain knowledge, but the core paradigm remains: scientists specify causal structure, ACE optimizes experimental validation.

\section{Limitations and Future Work}

Several limitations warrant discussion and suggest directions for future research.

\emph{Single-Run Reporting.} \textcolor{red}{[NOTE: This will be addressed when multi-seed experiments complete]} Our results currently report single-run outcomes; best practice requires multiple independent runs with confidence intervals and statistical significance testing. We are conducting 5 independent runs with different random seeds (42, 123, 456, 789, 1011) to provide proper statistical validation. Preliminary single-run results guide the experimental design, but final publication will include complete statistical analysis with error bars, confidence intervals, and p-values from paired t-tests.

\emph{Computational Requirements.} ACE requires high-fidelity simulations during policy training, with hundreds of episodes and thousands of forward evaluations needed before deployment; while inference is efficient (40--60 episodes), this training cost limits immediate application to domains with accurate simulators or sufficient historical data for offline learning---though industrial settings increasingly provide such capabilities through digital twins and process simulators. Our current implementation assumes the causal graph $\mathcal{G}$ is known, focusing on mechanism learning rather than structure discovery. This is a deliberate scope choice: mechanism estimation with known structure is itself a valuable problem (common in domains with prior knowledge), and allows focused evaluation of intervention design for functional learning. Extending ACE to jointly learn structure and mechanisms (combining our mechanism-focused approach with structure discovery methods like CORE or GACBO) remains an important direction for future work, potentially yielding a unified framework for complete causal discovery. The text-based encoding of graph topology faces context window limitations for large networks ($n > 20$ nodes), suggesting that future work should explore graph neural network architectures or structured representations that scale more efficiently. While our dedicated root learner improves upon naive approaches, exogenous distributions remain challenging for all methods tested, and theoretical analysis of sample complexity for learning root distributions under interventional regimes could guide further improvements. Finally, our results report single-run outcomes; future work should include multiple runs with confidence intervals and statistical significance testing to fully characterize the variance in training dynamics across random seeds.
