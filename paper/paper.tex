...
\begin{tikzpicture}[
    scale=1.0,
    node/.style={circle, draw, thick, minimum size=0.9cm, font=\normalsize},
    collider/.style={circle, draw, thick, minimum size=0.9cm, font=\normalsize, fill=red!15},
    arrow/.style={-{Stealth[length=3mm]}, very thick},
    edge_label/.style={font=\footnotesize, above, midway}
]

% Collider subgraph
\node[node] (X1) at (0,0) {$X_1$};
\node[node] (X2) at (2.5,0) {$X_2$};
\node[collider] (X3) at (5,0) {$X_3$};

\draw[arrow] (X1) -- node[edge_label] {linear} (X2);
\draw[arrow] (X2) -- node[edge_label] {nonlinear} (X3);
\draw[arrow] (X1) to[bend left=30] node[edge_label, above=2pt] {} (X3);

% Separate subgraph
\node[node] (X4) at (7.5,0) {$X_4$};
\node[node] (X5) at (10,0) {$X_5$};

\draw[arrow] (X4) -- node[edge_label] {quadratic} (X5);

% Annotations
\node[below=0.4cm of X3, font=\small, text=red!70!black] {collider};
