...
\icmltitlerunning{Active Causal Experimentalist: Learning Experimental Design via DPO}

\begin{document}

\twocolumn[
  \icmltitle{Active Causal Experimentalist: Learning Experimental Design via DPO}
...
\begin{abstract}
Discovering causal relationships requires controlled experiments, but which experiment should come next? We propose Active Causal Experimentalist (ACE), which learns to design experiments via Direct Preference Optimization. By comparing intervention outcomes pairwise rather than estimating value functions, ACE achieves stable training under non-stationary rewards. We introduce per-node convergence criteria and dedicated root learners to handle heterogeneous mechanism learning rates. Across synthetic benchmarks, physics simulations, and economic data, ACE achieves 70--71\% improvement over baselines at equal intervention counts (p<0.001, Cohen's $d \approx 2$). Ablation studies validate that each component contributes substantially (40--130\% degradation when removed). ACE without oracle initialization still outperforms all baselines, confirming that learned experimental design, not privileged information, drives performance.
\end{abstract}
...
\subsection{Oracle Pretraining: Necessary or Accelerant?}

To address concerns about the 200-intervention oracle warm-start providing privileged information, we evaluate ACE without any pretraining (pretrain\_steps=0) across five independent seeds. The results are instructive: ACE without oracle achieves 1.15 $\pm$ 0.22 total loss, representing 25\% degradation from oracle-pretrained ACE (0.92 $\pm$ 0.73, median 0.61). Critically, however, the no-oracle variant still substantially outperforms all baselines at matched budget (Random: 2.06, Round-Robin: 2.10, Max-Variance: 2.10), maintaining 46\% median improvement over the best baseline.

This validates that learned experimental design via DPO, not privileged initialization, drives ACE's advantage. The oracle warm-start accelerates convergence—reducing episodes from an average of 210 (no-oracle) to 171 (with oracle)—but is not necessary for superior performance. The policy learns to concentrate interventions on collider parents whether initialized from oracle demonstrations or from scratch, confirming that this strategic pattern emerges from the DPO training process itself rather than being inherited from the oracle.
...
ACE learns experimental design via Direct Preference Optimization, achieving 70--71\% improvement over baselines at equal episodes (171) with p<0.001 and Cohen's $d \approx 2$. Equal-episode comparisons isolate algorithmic contributions: baselines plateau at 2.06--2.10 regardless of episode count, while ACE reaches 0.61 median loss. Ablation studies validate that each architectural component contributes substantially (40--130\% degradation when removed). No-oracle ACE (1.15) still outperforms all baselines (2.06--2.10), confirming that learned design, not oracle initialization, drives performance. The policy discovers that concentrating interventions on collider parents is optimal, matching theoretical predictions through empirical learning.
