...
\textbf{Addressed limitations.} Equal-budget comparisons (171 episodes) demonstrate ACE's 70\% improvement persists independent of sample size. Random lookahead ablation (2.10 loss, comparable to baselines) confirms DPO-based proposal generation, not candidate evaluation alone, drives performance.

\textbf{Remaining limitations.} The 200-intervention oracle warm-start provides privileged information; ablations quantify this but headline results include it. With N=5 seeds, statistical power is limited and one outlier drives high variance. ACE assumes known structure, requires simulation access during training, and faces scalability limits beyond 20 nodes. The Phillips curve demonstrates strategic historical sampling but is observational data selection, not causal intervention in the do-calculus sense.
