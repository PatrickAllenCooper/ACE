...
def main():
    ...
    if args.log:
        ...
        if not matches:
            print(f"❌ No Phillips results found")
    ...
    with open(output_path, 'w') as f:
        ...
        if result['is_selective']:
            f.write(f"✅ Selective behavior detected. Paper claim is supported.\n\n")
            f.write(f"ACE queries high-volatility periods {result['selectivity']:.1f}x more than random sampling.\n")
        else:
            f.write(f"❌ No strong selectivity detected. Recommend revising paper claim.\n\n")
