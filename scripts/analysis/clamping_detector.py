...
def detect_clamping(experiment_log, target_node='X2', threshold=0.5, verbose=True):
    ...
    if verbose:
        ...
        if is_clamping:
            print(f"  [DETECTED] CLAMPING DETECTED")
            print(f"     Paper claim (line 661) SUPPORTED")
        else:
            print(f"  [NOT DETECTED] NO CLEAR CLAMPING")
...
def main():
    ...
    if args.log:
        ...
        if not matches:
            print(f"[ERROR] No Duffing logs found")
    ...
    with open(output_path, 'w') as f:
        ...
        if result['is_clamping']:
            ...
            f.write(f"[SUPPORTED] Clamping behavior detected. Paper claim is supported.\n")
        else:
            f.write(f"### Conclusion\n\n")
            f.write(f"[NOT SUPPORTED] No clear clamping pattern. Recommend revising paper claim.\n\n")
