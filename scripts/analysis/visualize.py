...
def create_success_dashboard(data, run_dir, output_path=None):
    ...
    print(f"[OK] Saved dashboard to: {output_path}")
    return output_path
...
Single-Parent Mechanisms:
  • X1 → X2 (Linear):     MSE = {mse_x2:.4f}  {'[OK]' if mse_x2 < 0.5 else '[FAIL]'}
  • X4 → X5 (Quadratic):  MSE = {mse_x5:.4f}  {'[OK]' if mse_x5 < 0.5 else '[FAIL]'}

Multi-Parent (Collider) Mechanism:
  • X1,X2 → X3:           MSE = {X3_error.mean()**2:.4f}  {'[OK]' if X3_error.mean() < 0.5 else '[FAIL]'}
...
"""
    ...
    print(f"[OK] Saved mechanism contrast to: {output_path}")
    return output_path
...
def print_summary(data):
    ...
    if 'node_losses' in data:
        ...
        for node in ['X1', 'X2', 'X3', 'X4', 'X5']:
            ...
            if col in final:
                loss = final[col]
                status = '[OK]' if loss < 0.5 else '[FAIL]'
